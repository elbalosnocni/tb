...
function login(employeeId, password) {
  ...
  if (!employeeId || !password) return {ok:false, message:'Vui lòng nhập mã nhân viên và mật khẩu.'};

  const emp = findEmployee_(employeeId);
  const success = !!emp && emp.Status === 'ACTIVE' && emp.Role && emp.PasswordHash &&
    hashPassword_(password, emp.PasswordSalt) === emp.PasswordHash;

  writeLoginHistory_(employeeId, success);

  if (!success) {
    return {ok:false, message:'Mã nhân viên hoặc mật khẩu không đúng, hoặc tài khoản không hoạt động.'};
  }
  ...
  CacheService.getScriptCache().put('SESSION_' + token, JSON.stringify(payload), CONFIG.sessionHours * 3600);
  ...
  return {
    ok:true,
    token:token,
    user:{employeeId:employeeId, fullName:emp.FullName, department:emp.Department, role:emp.Role}
  };
}
...
function getDashboard(token, filters) {
  ...
  const filtered = all.filter(a => {
    if (filters.month && !sameMonth_(a.PublishDate, filters.month)) return false;
...
function publicAdminAnnouncement_(a) {
  return {...publicAnnouncement_(a,false), scheduledAt:dateOut_(a.ScheduledAt),createdAt:dateOut_(a.CreatedAt)};
}
...
function requireAdmin_(session) {
  if(session.role !== 'ADMIN' && session.role !== 'HR') throw new Error('Không có quyền quản trị.');
}
...
function writeLoginHistory_(employeeId, success) {
  const sh=getSheet_(CONFIG.sheets.logins);
  sh.appendRow([
    'LOGIN-'+Utilities.getUuid().slice(0,8),employeeId,now_(),success,'',''
  ]);
}
