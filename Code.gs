...
function doPost(e) {
  try {
    ...
    const map = {
      login: function(){ return login(body.employeeId, body.userAgent); },
...
function setupSystem() {
  ...
  // EmployeeID/CCCD luôn lưu dạng TEXT để giữ số 0 ở đầu.
  const empSh = ss.getSheetByName(CONFIG.sheets.employees);
  if (empSh) empSh.getRange('A:A').setNumberFormat('@');
  const readsSh = ss.getSheetByName(CONFIG.sheets.reads);
  if (readsSh) readsSh.getRange('B:B').setNumberFormat('@');
  const targetsSh = ss.getSheetByName(CONFIG.sheets.targets);
  if (targetsSh) targetsSh.getRange('C:C').setNumberFormat('@');
...
function login(employeeId, userAgent) {
  employeeId = normalizeEmployeeId_(employeeId);
  if (!employeeId) return {ok:false, message:'Vui lòng nhập EmployeeID / số căn cước.'};

  const emp = findEmployee_(employeeId);
  const success = !!emp && String(emp.Status || '').toUpperCase() === 'ACTIVE';

  writeLoginHistory_(employeeId, success, userAgent);

  if (!success) {
    return {ok:false, message:'EmployeeID / số căn cước không đúng, không tồn tại hoặc tài khoản không hoạt động.'};
  }
...
function getAnnouncement(token, id) {
  ...
  const isAdmin = ['ADMIN','HR'].includes(String(session.role || '').toUpperCase());
...
function saveEmployee(token, data) {
  ...
  // Không sử dụng mật khẩu cho bất kỳ tài khoản nào.
  sh.appendRow([id,name,String(data.department||''),String(data.position||''),String(data.email||''),
    String(data.phone||''),'','',role,status,now,now,'']);
  ...
  rows.forEach(r=>{
    ...
    if (!old) {
      sh.appendRow([
        ...
        '','',r.Role || 'EMPLOYEE','ACTIVE',now_(),now_(),''
      ]);
      added++;
    } else {
      const row = [
...
function normalizeEmployeeId_(id) {
  ...
  // CCCD/EmployeeID là chuỗi số; không dùng parseInt vì sẽ làm mất số 0 đầu.
  return s.toUpperCase();
}
function findEmployee_(id) {
  const wanted = normalizeEmployeeId_(id);
  return getEmployeeObjects_().find(e => normalizeEmployeeId_(e.EmployeeID) === wanted) || null;
}
...
function requireAdmin_(session) {
  if(!['ADMIN','HR'].includes(String(session.role || '').toUpperCase())) {
...
function normalizeEmployeeImportRows_(rows) {
  ...
  return rows.map(r=>{
    ...
    return {
      EmployeeID:normalizeEmployeeId_(pick(['EmployeeID','Mã nhân viên','Ma nhan vien','Mã NV','Ma NV'])), 
